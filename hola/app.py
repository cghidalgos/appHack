...
# Ruta para subir varios archivos
@app.route('/admin/upload_multiple', methods=['GET', 'POST'])
def admin_upload_multiple():
    print("asads")
    if session.get('role') != 'admin':
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if not files or len(files) == 0:
            ...
            return redirect(url_for('admin_upload_multiple'))
        
        if len(files) > 5:
            flash('Solo se permiten máximo 5 archivos por carga.')
            return redirect(url_for('admin_upload_multiple'))

        resultados = []  # para guardar resultados por archivo
        ...
        for file in files:
            ...
            if nombre and cedula:
                ...
                resultados.append({'archivo': filename, 'nombre': nombre, 'cedula': cedula, 'status': '✅ Éxito'})
            else:
                resultados.append({'archivo': filename, 'status': '❌ Falló extracción'})

        flash('Carga completada. Revisa los resultados abajo.')
        return render_template('admin_upload_multiple.html', resultados=resultados)

    return render_template('admin_upload_multiple.html', resultados=None)
